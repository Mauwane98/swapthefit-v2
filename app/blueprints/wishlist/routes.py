...
def view_wishlist():
    ...
    """
    wishlist_items = list(mongo.db.wishlist.find({'user_id': current_user.id}))
    for item in wishlist_items:
        item['listing'] = mongo.db.listings.find_one({'_id': ObjectId(item['listing_id'])})
...
def add_to_wishlist(listing_id):
    ...
    """
    listing = mongo.db.listings.find_one_or_404({'_id': ObjectId(listing_id)})
    
    # Check if the item is already in the wishlist
    existing_item = mongo.db.wishlist.find_one({'user_id': current_user.id, 'listing_id': listing_id})
...
def remove_from_wishlist(wishlist_item_id):
    ...
    """
    wishlist_item = mongo.db.wishlist.find_one_or_404({'_id': ObjectId(wishlist_item_id)})
    ...
    mongo.db.wishlist.delete_one({'_id': ObjectId(wishlist_item_id)})
