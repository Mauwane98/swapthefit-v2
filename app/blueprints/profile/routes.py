...
from app.services.recommendation_service import RecommendationService # Add this import
...
def profile():
    ...
    disputes_initiated = Dispute.objects(initiator=current_user.id).count()
    disputes_responded = Dispute.objects(respondent=current_user.id).count()
    disputes_resolved_in_favor = Dispute.objects(Q(initiator=current_user.id, resolution='resolved_in_favor_of_initiator') | Q(respondent=current_user.id, resolution='resolved_in_favor_of_respondent')).count()
    disputes_at_fault = Dispute.objects(Q(initiator=current_user.id, resolution='resolved_in_favor_of_respondent') | Q(respondent=current_user.id, resolution='resolved_in_favor_of_initiator')).count()

    # Fetch completed transactions
    completed_swaps = SwapRequest.objects(Q(requester=current_user.id) | Q(responder=current_user.id), status='completed').order_by('-updated_date')
    completed_orders = Order.objects(Q(buyer=current_user.id) | Q(seller=current_user.id), status='completed').order_by('-created_at')
    completed_donations = Donation.objects(Q(donor=current_user.id) | Q(recipient=current_user.id), status='completed').order_by('-created_at')
    ...
    badge_service.check_and_award_badges(current_user)
    ...
    return render_template(
        ...
        disputes_responded=disputes_responded,
        disputes_resolved_in_favor=disputes_resolved_in_favor,
        disputes_at_fault=disputes_at_fault,
        completed_swaps=completed_swaps,
        completed_orders=completed_orders,
        completed_donations=completed_donations,
        user_badges=user_badges, # Pass user badges to the template
        recommended_listings=recommended_listings # Pass recommended listings
    )
...
def payout_details():
    """
    Allows sellers (users with role 'school', 'ngo', or 'parent') to enter and update their bank details for payouts.
    """
    if not (current_user.has_role('school') or current_user.has_role('ngo') or current_user.has_role('parent')):
...
@profile_bp.route("/api/mark_onboarding_complete", methods=['POST'])
@login_required
def mark_onboarding_complete():
    """
    API endpoint to mark the user's onboarding as complete.
    """
    try:
        ...
        current_app.logger.error(f"Error marking onboarding complete for user {current_user.id}: {e}")
        return jsonify({'success': False, 'message': 'Failed to mark onboarding complete.'}), 500
